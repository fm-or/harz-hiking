...
class GraphData:
        ...
        if self._stamp_points is None:
            with self._neo4j_session() as session:
                query = "MATCH (n:StampPoint) " \
                        "RETURN elementId(n) AS neo4j_id, n.latitude AS latitude, n.longitude AS longitude, n.stamp_id AS stamp_id, n.name AS name"
        ...
        if self._bus_stops is None:
            with self._neo4j_session() as session:
                query = "MATCH (n:BusStop) " \
                        "RETURN elementId(n) AS neo4j_id, n.latitude AS latitude, n.longitude AS longitude"
        ...
        if self._parking_lots is None:
            with self._neo4j_session() as session:
                query = "MATCH (n:ParkingLot) " \
                        "RETURN elementId(n) AS id, n.latitude AS latitude, n.longitude AS longitude"
                self._parking_lots = {result.get("id"): ParkingLot(result.get("id"),
                                                                    result.get("latitude"),
                                                                    result.get("longitude"))
                                      for result in session.run(query)}
    ...
    def import_data(self,
        ...
        if self._map_exists():
            ...
            if new_stamp_point_count > 0 and output:
    ...
    def _import_osm_entities(self,
        ...
        with self._neo4j_session() as session:
            ...
            thinned_indizes = list()
            deleted_indizes = list()
    ...
    def _import_missing_distances(self, max_section_length_m: Optional[float]) -> int:
        with self._neo4j_session() as session:
            ...
            origin_osmids = list()
            destiation_osmids = list()
            relation_osmids = list()
            ...
            if len(origin_osmids) > 0:
                routes = ox.routing.shortest_path(
                    self._map, orig=origin_osmids, dest=destiation_osmids,
            ...
            if len(origins) > 0:
                routes = ox.routing.shortest_path(
                    self._map, orig=origins, dest=destinations, weight='length', cpus=self._threads)
                for i, route in enumerate(routes):
                    if len(route) == 1:
                        distance = 0
                    else:
            ...
            if len(origin_osmids) > 0:
                routes = ox.routing.shortest_path(
                    self._map, orig=origin_osmids, dest=destiation_osmids, weight='length', cpus=self._threads)
                for i, route in enumerate(routes):
                    if len(route) == 1:
                        distance = 0
                    else:
